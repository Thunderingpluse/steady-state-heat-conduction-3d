        ...
        for i in range(ny):
            for j in range(nx):
                ...
                if faces_temp:
                    ...
                    eq = f"{b_center:.1f}*T({j},{i},{k}) + {bx:.1f}*T({j-1},{i},{k}) + {bx:.1f}*T({j+1},{i},{k}) + {by:.1f}*T({j},{i-1},{k}) + {by:.1f}*T({j},{i+1},{k}) + {bz:.1f}*T({j},{i},{k-1}) + {bz:.1f}*T({j},{i},{k+1})"
                    rhs = 0
                
                print(f"{node_str:<15} | {eq:<110} | {rhs:<12.2f}")

    T_flat = np.linalg.solve(A, B)
    T = T_flat.reshape((nz, ny, nx))

    # Calculate mid-z slice for visualization
